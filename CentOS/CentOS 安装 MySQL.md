...
下载`mysql-5.7.23-linux-glibc2.12-x86_64.tar.gz`。
...
basedir=/opt/mysql/mysql-5.7.23_3306
...
```shell
# 初始化数据库，加参数-insecure则不会生成随机密码
> ./bin/mysqld --defaults-file=/opt/mysql/config/config-3306.cnf --initialize-insecure
# 启动数据库
> ./bin/mysqld --defaults-file=/opt/mysql/config/config-3306.cnf
# 配置密码
> ./bin/mysqladmin -u root password "Root4510@" --socket=/opt/mysql/sock/3306.sock
# 连接数据库
...
lockdir=/opt/mysql/data
lock_file_path=$lockdir/mysql

# The following variables are only set for letting mysql.server find things.

# Set some defaults
mysqld_pid_file_path=/opt/mysql/data/3306/c-128.pid
if test -z "$basedir"
then
  basedir=/opt/mysql/mysql-5.7.23_3306
  bindir=/opt/mysql/mysql-5.7.23_3306/bin
  if test -z "$datadir"
  then
    datadir=/opt/mysql/data/3306
  fi
  sbindir=/opt/mysql/mysql-5.7.23_3306/bin
  libexecdir=/opt/mysql/mysql-5.7.23_3306/bin
else
  bindir="$basedir/bin"
  if test -z "$datadir"
  then
...
1. [centos7单机上搭建mysql5.7一主多从模式](https://www.cnblogs.com/hankguo/p/8204868.html)
2. [Linux下设置MySql自动启动](https://www.cnblogs.com/sunny3096/p/7954146.html)
